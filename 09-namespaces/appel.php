...
namespace General;      // ne marche plus si on met un namespace genéral ici ! (PHP +- la gde armoire)
// alors =>   
use PDO;
use Espace1;
use Espace2;
...
$c = new Espace1\A;         // du coup obligé d'appeler class par/dans son espace virtuel
echo $c -> test() . '<hr>';
...
/*****************⚠️ COMMENTAIRES ⚠️*******************
    ...
    Les namespaces permettent de DECLARER DES ESPACES VIRTUELS afin de mieux organiser nos fichiers, 
    ...
    Dès lors qu'on utilise les namespaces qques régles s'appliquent :
        ...
        - pour pvr utiliser les class d'un autre namespace ou de l'espace global de PHP (PDO par ex), on doit au choix :
            -> Dévinir le chemin complet :⚠️ $PDO = new \PDO
